...
## model.json (schema snapwright.model/0.2)
...
`stats.failures`. `stats.necks[]` {plate, strength (studs), parts_above, mass_g, cut
[[lower, upper] part ids]}. Change counts: recolored_cells, trimmed_cells, added_cells,
studded_cells; floating_voxels, design_voxels. Schema 0.1 files are upgraded on read
(`pipeline.load_model`).
