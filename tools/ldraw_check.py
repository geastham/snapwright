...
def check(model, ldr_text, lib, cat):
    parts = model["parts"]
    lines = [l.split() for l in ldr_text.splitlines() if l.startswith("1 ")]
    order = [pid for st in model["steps"] for pid in st["parts"]]
        ...
        if t.shape in ("slope", "slope_inv"):
            ...
            for tri, s in tris:
                ...
                if hz < 0.3 or abs(n[1]) < 0.3:
                    continue
                facing_up = n[1] < 0                      # -Y is up in LDraw
                key = (round(n[0] / hz), round(n[2] / hz), facing_up)
                area[key] = area.get(key, 0) + a
            # LDraw winding isn't always outward, so take the slanted plane with the most area
            # and orient it outward: up-facing for slopes, down-facing for inverted slopes
            if area:
                (ax, az, up_), _ = max(area.items(), key=lambda kv: kv[1])
                if up_ != (t.shape == "slope"):
                    ax, az = -ax, -az
                if (ax, az) != want:
                    problems.append(f"part {pid} {p['part']} dir {p['dir']}: sloped face points {(ax, az)} "
                                    f"in LDraw, expected {want}")
...
def main():
    ...
    for p in problems[:40]:
        print("PROBLEM:", p)
    print(f"{len(model['parts'])} parts checked against the official LDraw geometry: "
